...
def setup_launch(context,*args, **kwargs) :
    ...
    logger = get_logger("spawn_chaser_launch")
    ...
    args = ['-topic', '/'+model_name + '/robot_description', '-entity', model_name, '-z', '10.0','-x', '10.0','-y', '10.0'] # if you did not name the robot correctly in the model.xacro file, then nothing will be shown
    spawn_entity = Node(
        package='gazebo_ros',
...
def generate_launch_description():
    ...
    model_name_arg = DeclareLaunchArgument(name='model',default_value='cubot',description='Name of the model to spawn contained in the description folder (without the .xacro in the end, just the ma,e of the foler is sufficient). Default to the ISS') # declaration of the launch argument
    ld += [model_name_arg]
